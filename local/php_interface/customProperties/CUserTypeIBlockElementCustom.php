...
AddEventHandler('main', 'OnUserTypeBuildList', array('CUserTypeIBlockElementCustom', 'GetUserTypeDescription'), 5000);
class CUserTypeIBlockElementCustom {
	// ---------------------------------------------------------------------
	...
	static function GetUserTypeDescription() {
		return array(
			...
			'USER_TYPE_ID' => 'iblock_element_custom',
			// имя класса, методы которого формируют поведение типа
			'CLASS_NAME' => 'CUserTypeIBlockElementCustom',
	...
	static function GetEditFormHTML($arUserField, $arHtmlControl) {
		...
		$arElements = CUserTypeIBlockElementCustom::_getElements($arHtmlControl['VALUE']);
		// html поля веб-формы для текущего значения
		$sReturn .= '<div>'.CUserTypeIBlockElementCustom::_getItemFieldHTML($arHtmlControl['VALUE'], $iIBlockId, $arElements, $arHtmlControl['NAME']).'</div>';
		return $sReturn;
	}

	// Функция вызывается при выводе формы редактирования значения множественного свойства
	// @return string - HTML для вывода
	static function GetEditFormHTMLMulty($arUserField, $arHtmlControl) {
		$iIBlockId = intval($arUserField['SETTINGS']['IBLOCK_ID']);
		...
		foreach($arHtmlControl['VALUE'] as $iKey => $iValue) {
			$sReturn .= '<tr><td><div>'.CUserTypeIBlockElementCustom::_getItemFieldHTML($iValue, $iIBlockId, $arElements, $arHtmlControl['NAME']).'</div></td></tr>';
		}
		// html поля веб-формы для добавления нового значения свойства
		$sReturn .= '<tr><td><div>'.CUserTypeIBlockElementCustom::_getItemFieldHTML(0, $iIBlockId, array(), $arHtmlControl['NAME']).'</div></td></tr>';
		// html полей для кнопки Добавить... (режим множественного выбора элементов)
		$sReturn .= '<tr><td><div>'.CUserTypeIBlockElementCustom::_getItemFieldHTML(0, $iIBlockId, array(), $arHtmlControl['NAME'], 'y').'</div></td></tr>';
		$sReturn .= '</table>';
		return $sReturn;
	}
	...
	static function GetFilterHTML($arUserField, $arHtmlControl) {
		...
		return CUserTypeIBlockElementCustom::GetEditFormHTML($arUserField, $arHtmlControl);
	}
	...
	static function GetAdminListViewHTML($arUserField, $arHtmlControl) {
		...
		if($iElementId > 0) {
			$arElements = CUserTypeIBlockElementCustom::_getElements($iElementId);
	...
	// @return string - HTML для вывода
	static function GetAdminListViewHTMLMulty($arUserField, $arHtmlControl) {
		$sReturn = '';
		static $bWasJs = false;
	...
	// @return string - HTML для вывода
	// @private
	static function _getItemFieldHTML($iValue, $iIBlockId, $arElements, $sFieldName, $sMulty = 'n') {
		$sReturn = '';
	...
	// @private
	static function _getElements($mElementId = array()) {
		$arReturn = array();
